...
void axk_kheap_debug( void )
{
    ...
    // Print header
    axk_terminal_lock();
    axk_terminal_prints( "===================> Heap State <=====================\n" );
    ...
    while( ptr_iter <= g_highest_tag )
    {
        ...
        axk_terminal_printh64_lz( (uint64_t)ptr_iter, true );
        axk_terminal_prints( ":\n\t\tPresent: " );
        axk_terminal_prints( b_present ? "YES" : "NO" );
        axk_terminal_prints( "  Begin: " );
        axk_terminal_prints( b_begin ? "YES" : "NO" );
        axk_terminal_prints( "  End: " );
        axk_terminal_prints( b_end ? "YES" : "NO" );
        axk_terminal_prints( "\n\t\tNext: " );
        axk_terminal_printh64( next );
        axk_terminal_prints( " Prev: " );
        axk_terminal_printh64( prev );
        axk_terminal_printnl();
        
        // Check for the end of the heap
        if( next == AXK_KERNEL_VA_HEAP )
        {
            axk_terminal_prints( "=================> Heap End <=================\n" );
            break;
        }
    ...
    axk_terminal_prints( "\n\tPages: " );
    axk_terminal_printu64( g_kheap_pages );
    axk_terminal_prints( "  Lowest Avail: " );
    axk_terminal_printh64( (uint64_t) g_lowest_tag );
    axk_terminal_printnl();
    axk_terminal_unlock();
