...
bool axk_x86_calibrate_tsc_driver( struct axk_timer_driver_t* self )
{
    ...
    if( cpuid_tsc_frequency > 0UL )
    {
        ...
        if( diff > observed / 100UL ) 
        {
            ...
            axk_terminal_prints( "TSC (x86): Warning, the observed TSC frequency varies from the rate calculated from CPUID\n" );
            axk_terminal_prints( "\t\t Observed: " );
            axk_terminal_printu64( observed );
            axk_terminal_prints( " Hz   CPUID: " );
            axk_terminal_printu64( cpuid_tsc_frequency );
            axk_terminal_prints( " Hz, going to use the observed frequency\n" );
        }
        else
        {
            this->frequency = cpuid_tsc_frequency;

            axk_terminal_prints( "TSC (x86): Acquired TSC rate from CPUID, running at " );
            axk_terminal_printu64( cpuid_tsc_frequency );
    ...
    else
    {
        ...
        axk_terminal_prints( "TSC (x86): Using observed TSC rate, running at approx. " );
