...
bool xapic_driver_set_external_routing( struct axk_interrupt_driver_t* this_generic, struct axk_external_interrupt_routing_t* routing )
{
    ...
    if( lapic_id > 0b1111 ) 
    {
        axk_terminal_prints( "xAPIC Driver: [Warning] Attempt to route external interrupt to a processor with an out-of-bounds LAPIC identifier (Available: [0,15]) [Selected: " );
        axk_terminal_printu32( (uint32_t) lapic_id );
        axk_terminal_prints( ")\n" );
        return false;
    }
    ...
    if( target_ioapic == NULL )
    {
        ...
        axk_terminal_printu32( ext_number );
        axk_terminal_prints( ")\n" );
