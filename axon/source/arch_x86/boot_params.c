...
bool axk_x86_bootparams_parse( void* ptr )
{
    // Check for null, shouldnt happen though because we validated in ASM
    struct tzero_payload_params_t* ptr_params = (struct tzero_payload_params_t*)( ptr );
    if( ptr_params == NULL || ptr_params->framebuffer_ptr == NULL || ptr_params->rsdp_ptr == NULL ) { return false; }
    ...
    if( !parse_framebuffer( pparams ) ) { return false; }

    // Parse the memory map into an arch-idependent structure
    if( !parse_memorymap( pparams ) ) { return false; }
