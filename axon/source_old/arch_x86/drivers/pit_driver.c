...
uint32_t pit_start( struct axk_timer_driver_t* self, enum axk_timer_mode_t mode, uint64_t delay, bool b_delay_in_ticks, uint32_t processor, uint8_t vector )
{
    ...
    if( mode == AXK_TIMER_MODE_ONE_SHOT )
    {
        ...
        if( ticks > 0xFFFFUL )
        {
            axk_basicout_prints( "PIT (x86): [Warning] Attempt to use 'one-shot' mode, but delay exceeds the max allowed delay due to hardware limitations (54.9 ms or 65535 ticks)\n" );
            axk_spinlock_release( &( this->lock ) );
            return AXK_TIMER_ERROR_INVALID_PARAMS;
        }
    ...
    else // AXK_TIMER_MODE_DIVISOR
    {
        ...
        if( delay > 0xFFFFUL )
        {
            axk_basicout_prints( "PIT (x86): [Warning] Attempt to use 'divisor' mode, but the divisor is out of bounds (Max is 65535)\n" );
            axk_spinlock_release( &( this->lock ) );
            return AXK_TIMER_ERROR_INVALID_PARAMS;
        }
