...
bool xapic_driver_init( struct axk_interrupt_driver_t* this_generic )
{
    ...
    this->lapic_count               = ptr_acpi->lapic_count;
    ...
    this->lapic_list            = AXK_CALLOC_TYPE( struct axk_x86_lapic_info_t, this->lapic_count );
    ...
    memcpy( (void*) this->lapic_list, (void*) ptr_acpi->lapic_list, sizeof( struct axk_x86_lapic_info_t ) * this->lapic_count );
    ...
    for( uint32_t i = 0; i < this->ioapic_count; i++ )
    {
        ...
        ptr_ioapic->interrupt_count = (uint8_t)( ( axk_x86_xapic_read_ioapic( this, ptr_ioapic->id, IOAPIC_REGISTER_VERSION ) & 0x00FF0000U ) >> 16 ) + 1;
        index++;
    }
...
bool xapic_driver_set_external_routing( struct axk_interrupt_driver_t* this_generic, struct axk_external_interrupt_routing_t* routing )
{
    ...
    {
        axk_basicout_prints( "xAPIC Driver: [Warning] Attempt to set external routing with an invalid processor ID\n" );
        axk_basicout_prints( "====> " ); axk_basicout_printu32( routing->target_processor );
        axk_halt();
        return false;
    }
    ...
    if( lapic_id > 0b1111 ) 
    {
        axk_basicout_prints( "xAPIC Driver: [Warning] Attempt to route external interrupt to a processor with an out-of-bounds LAPIC identifier (Available: [0,15]) [Selected: " );
        axk_basicout_printu32( (uint32_t) lapic_id );
        axk_basicout_prints( ")\n" );
        return false;
    }
    ...
    {
        axk_basicout_prints( "xAPIC Driver: [Warning] Attempt to set external routing for an out-of-bounds external interrupt vector (" );
        axk_basicout_printu32( ext_number );
        axk_basicout_prints( ")\n" );
...
bool xapic_driver_clear_external_routing( struct axk_interrupt_driver_t* self, uint32_t global_interrupt )
{
    ...
    if( target_ioapic == NULL )
    {
        axk_basicout_prints( "xAPIC Driver: [Warning] Attempt to clear external routing for an out-of-bounds external interrupt vector\n" );
        return false;
    }
...
void axk_x86_xapic_init_ioapic( struct axk_x86_xapic_driver_t* this, struct axk_x86_ioapic_nmi_t* nmi_list, uint32_t nmi_count )
{
    ...
    for( uint32_t i = 0; i < nmi_count; i++ )
    {
        ...
        if( !xapic_driver_set_external_routing( (struct axk_interrupt_driver_t*)this, &route ) )
        {
            axk_basicout_prints( "xAPIC Driver: [Warning] Failed to setup an external NMI!\n" );
        }
    }
}
...
void axk_x86_xapic_write_ioapic( struct axk_x86_xapic_driver_t* this, uint8_t id, uint8_t reg, uint32_t val )
{
    ...
    if( ptr_reg == NULL )
    {
        axk_basicout_prints( "xAPIC Driver: [ERROR] Attempt to access an invalid IOAPIC\n" );
        return;
    }
...
void axk_x86_xapic_write_lapic( struct axk_x86_xapic_driver_t* this, uint32_t reg, uint32_t value )
{
    if( reg > 0x3F0 ) 
    { 
        axk_basicout_prints( "xAPIC Driver: [ERROR] Attempt to access invalid local APIC register\n" );
        return;
    }
