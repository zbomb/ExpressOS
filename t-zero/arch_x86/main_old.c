...
static UINTN g_memorymap_key = 0;
/*
    GUIDS
*/
...
/*
    Callback: On Launch Success
*/
void tzero_callback_success( void )
{
    g_sys_table->BootServices->ExitBootServices( g_img_handle, g_memorymap_key );
}

/*
    Callback: On Launch Error
*/
void tzero_callback_error( const char* str_header, const char* str_info )
{
    tzero_console_clear();
    tzero_console_prints( L"==> T-0: Failed to launch payload! The error reported was as follows: \r\n" );
    tzero_console_prints( L"\r\n     " );

    if( str_header != NULL )
    {
        // Convert the ASCII error string to UTF-16
        size_t header_len = strlen( str_header ) + 1UL;
        uint16_t* header_str = tzero_alloc_data( header_len * 2UL );
        for( size_t i = 0; i < header_len; i++ )
        {
            header_str[ i ] = str_header[ i ];
        }

        tzero_console_prints( (CHAR16*) header_str );
        tzero_free_data( header_str );

        if( str_info != NULL )
        {
            header_len = strlen( str_info ) + 1UL;
            header_str = tzero_alloc_data( header_len * 2UL );
            for( size_t i = 0; i < header_len; i++ )
            {
                header_str[ i ] = str_info[ i ];
            }

            tzero_console_prints( L"\r\n" );
            tzero_console_prints( L"\r\n" );
            tzero_console_prints( L"    Additional Info: " );
            tzero_console_prints( (CHAR16*) header_str );
            tzero_console_prints( L"\r\n" );
        }
    }
...
EFI_STATUS efi_main( EFI_HANDLE in_handle, EFI_SYSTEM_TABLE* in_table )
{
    ...
    */
    ...
    UINTN memdesc_size, map_size;
    UINT32 memdesc_ver;

    g_sys_table->BootServices->GetMemoryMap( &map_size, ptr_memdesc, &g_memorymap_key, &memdesc_size, &memdesc_ver );
    ...
    */
    ...
    params.fn_load_success          = tzero_callback_success;
    params.fn_load_failed           = tzero_callback_error;
